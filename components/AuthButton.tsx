...
export default function AuthButton() {
  ...
  return (
    <div className={styles.userWrap} ref={dropRef}>
      <button className={styles.userBtn} onClick={() => setOpen(p => !p)}>
        {avatarUrl ? (
      ...
      {open && (
        <div className={styles.dropdown}>
          <div className={styles.dropHeader}>
            {avatarUrl ? (
              <img src={avatarUrl} alt={name} className={styles.dropAvatar} referrerPolicy="no-referrer" />
            ) : (
              <div className={styles.avatarFallback}><User size={16} /></div>
            )}
            <div>
          ...
          <Link href="/favourites" className={styles.dropItem} onClick={() => setOpen(false)}>
            <Heart size={15} /> My Favourites
          </Link>
          <div className={styles.dropDivider} />
          <button className={`${styles.dropItem} ${styles.signOutItem}`} onClick={() => { signOut(); setOpen(false); }}>
